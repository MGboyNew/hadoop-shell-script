...
echo -e "\n..................修改配置文件(/etc/profile)............................."
echo_msg=("export SPARK_HOME=$tarFilePath\n" "export PATH=\$PAHT:$tarFilePath\/bin:\$PATH\n")
echoMsg "${echo_msg[*]}" "\nadd to /etc/profile >> \n"

lines=("#path spark\n""export SPARK_HOME=$tarFilePath\n""export PATH=\$PAHT:$tarFilePath\/bin:\$PATH\n")
...
echo -e "\n..................修改spark配置文件(sparn-env.sh,slaves)............................."

defaultEnv="$tarFilePath/conf/spark-env.sh.template"
env="$tarFilePath/conf/spark-env.sh"
slavesFile="$tarFilePath/conf/slaves"

cp $defaultEnv  $env
if [[ $? == 1 ]];
then
    echo "未找到默认配置文件$tarFilePath/conf/spark-env.sh.template"
    exit 1
fi

#spark-env.sh
lines=("export JAVA_HOME=$JAVA_HOME\n""export SCALA_HOME=$SCALA_HOME\n""SPARK_MASTER_HOST=$hostname\n")
echoFile "${lines[*]}" $env 

#slaves
echo `hostname` >> $slavesFile
echoFile "${hostnames[*]}" $slavesFile
